...
import {
  Activity, ClipboardList, CalendarDays, TrendingUp,
  Ruler, Target, Dumbbell,
} from "lucide-react";
...
export function ClientTabs({ client, progress, routines, appointments, t }: TabProps) {
  const [activeTab, setActiveTab] = useState("overview");

  const tabs = [
    { id: "overview", label: t.overview, icon: Activity },
    { id: "progress", label: t.progress, icon: TrendingUp },
    { id: "routines", label: t.routines, icon: ClipboardList },
    { id: "appointments", label: t.appointments, icon: CalendarDays },
  ];

  const activeRoutine = routines.find((r: any) => r.client_id === client.id);
      ...
      {activeTab === "overview" && (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              <h3 className="mb-3 text-sm font-semibold">{t.activeRoutine}</h3>
              {activeRoutine ? (
                <div>
                  <p className="text-sm font-medium">{activeRoutine.name}</p>
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              <h3 className="mb-3 text-sm font-semibold">{t.lastSession}</h3>
              {lastAppointment ? (
                <div>
                ...
                <p className="text-sm text-muted-foreground">Sin sesiones previas</p>
              )}
            </div>
          </div>
        </div>
                ...
                {routine.exercises && routine.exercises.length > 0 && (
                  <div className="space-y-2">
                    {routine.exercises.map((re: any) => (
                      <div key={re.id} className="flex items-center justify-between rounded-lg bg-background px-4 py-2.5 text-sm">
                        <div className="flex items-center gap-3">
                          <Dumbbell className="h-4 w-4 text-muted-foreground" />
                          <span>{re.exercise?.name || "Ejercicio"}</span>
                        </div>
                        <div className="flex gap-3 text-xs text-muted-foreground">
                          {re.sets && <span>{re.sets} series</span>}
                          {re.reps && <span>{re.reps} reps</span>}
                          {re.rest_time && <span>Desc: {re.rest_time}s</span>}
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {activeTab === "appointments" && (
        <div className="rounded-xl border border-border bg-card p-6">
          {appointments.length === 0 ? (
            <p className="text-center text-sm text-muted-foreground">Sin sesiones registradas</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
