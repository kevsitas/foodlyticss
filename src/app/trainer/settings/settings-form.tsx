...
import { updateMyTrainer } from "@/app/actions/trainers";
...
export function SettingsForm({ profile, trainer, t }: SettingsFormProps) {
  const [state, formAction, pending] = useActionState(
    async (_prev: FormState, form: FormData) => {
      ...
      const result = await updateMyTrainer({
        specialty: form.get("specialty") as string || null,
        certifications: form.get("certifications") as string || null,
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">{t.certifications}</label>
              <input
                name="certifications"
          ...
          <div>
            ...
            <input
              ...
              type="number"
              defaultValue={trainer?.experience_years ?? ""}
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
            />
          </div>
          <div>
